...
resource "aws_s3_bucket_acl" "s3_pre_pro" {
  depends_on = [aws_s3_bucket_ownership_controls.s3_pre_pro]
...
resource "aws_s3_bucket_policy" "s3_pre_pro" {
  depends_on = [
    aws_s3_bucket.s3_pre_pro,
    aws_s3_bucket_acl.s3_pre_pro,
    aws_s3_bucket_website_configuration.s3_pre_pro,
    data.aws_iam_policy_document.s3_pre_pro,
  ]
